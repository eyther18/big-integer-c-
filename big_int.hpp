...
private:
    ...
    void remove_lead_zeros(string &val)
    {
        int cur = 0;
        int len = val.length();
        while (cur < len)
    ...
    {
        big_int result;
        string res = "0", a = this->val, b = num.val;
    ...
    pair<big_int, big_int> div_mod(big_int num)
    {
        if (num.is_zero())
...
public:
    ...
    big_int(int num) : big_int(to_string(num)) {}

    big_int(long long num) : big_int(to_string(num)) {}
    ...
    {
        if (this->is_negative)
            this->val.insert(this->val.begin(), '-');
        cout << (this->val.empty() ? "0" : this->val);
    }
    ...
    big_int add(big_int num)
    {
        ...
        while (cur_a >= 0 || cur_b >= 0 || carry)
        {
            int cur_sum = 0;
            if (cur_a >= 0)
                cur_sum += (a[cur_a] - '0'), cur_a--;
            if (cur_b >= 0)
                cur_sum += (b[cur_b] - '0'), cur_b--;
            cur_sum += carry;
    ...
    big_int divide(big_int num)
    {
        ...
        pair<big_int, big_int> qr = this->div_mod(num);
    ...
    /////////////// I/O stream /////////////////
    friend ostream &operator<<(ostream &, big_int);
    friend ostream &operator>>(ostream &, big_int &);
    //////////////RELATIONAL OPERATOR///////////////////
...
istream &operator>>(istream &in, big_int &num)
{
    num.read();
    return in;
}

ostream &operator<<(ostream &out, big_int num)
{
    num.print();
    return out;
}
